...
from settings import WEB_ROOT, HTTP_SOURCE_ADDR, GARMIN_CONNECT_USER_WATCH_ACCOUNTS
...
class GarminConnectService():
    ...
    def __init__(self, email, password):
        ...
        cachedHierarchy = None
        self.email=email
        self.password=password
        
        if not cachedHierarchy:
            rawHierarchy = requests.get("https://connect.garmin.com/proxy/activity-service-1.2/json/activity_types", headers=None).text
    ...
    def _rate_limit(self): 
        import time
        min_period=1
        wait_time = min_period
        time.sleep(wait_time)
            
    def _rate_limit_old(self):
        #import fcntl, ruct, time
    ...
    def _request_with_reauth(self, req_lambda, email=None, password=None):
        ...
        for i in range(self._reauthAttempts + 1):
            session = self._get_session(email=email, password=password, skip_cache=i > 0)
            self._rate_limit()
    ...
    def _get_session(self, email=None, password=None, skip_cache=False):            
        ...
        cached=False
        record=None # we are not using databases
    ...
    def Authorize(self, email=None, password=None):
        #from tapiriik.auth.credential_storage import CredentialStore
        #from credential_storage import CredentialStore
    ...
    def DeleteActivity(self, serviceRecord, uploadId):
        ...
        session = self._get_session(record=None)
        self._rate_limit()
